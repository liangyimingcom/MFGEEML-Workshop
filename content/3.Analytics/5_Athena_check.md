...
~~~
SELECT COUNT(1) as TotalCount FROM "e2e-workshop"."e2e_sample_data_lab2_csv";
~~~
...
~~~
SELECT COUNT (1) as TotalCount FROM "e2e-workshop"."e2e_sample_data_lab2_csv" where code='500304';
~~~

结果窗口中将显示表 ‘e2e_sample_data_lab2_csv’ 中错误代码为500304的记录条数。请记录本次查询的运行时间和扫描数据量。您应该看到类似下面的结果：(运行时间: 1.17 秒, 扫描的数据: 48.54 MB)。
